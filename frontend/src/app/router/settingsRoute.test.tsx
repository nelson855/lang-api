...
vi.mock('../../features/auth/authState', async (importOriginal) => {
  ...
  return {
    ...actual,
    useAuthProfile: () => ({ id: 42, username: 'ordinary', displayName: 'Ord', email: 'a@example.test' }),
  };
});
...
describe('个人设置路由与导航', () => {
  it('路由元数据落在控制台守卫下且标题键存在', () => {
    const meta = ROUTE_META.find((item) => item.id === 'settings');
    ...
    expect(typeof (zhCN.pages as Record<string, unknown>)['settings']).toBe('object');
  });
  ...
  it('登录后可见标题与高亮导航', async () => {
    ...
    await waitFor(() => {
      expect(document.title).toContain('个人设置');
    });
  });

  it('未登录带安全回跳且不泄露上游路径', async () => {
    const router = setup('anonymous', '/dashboard/settings');
    await waitFor(() => {
      expect(router.state.location.pathname).toBe('/login');
    });
    const search = router.state.location.search;
    expect(search).toContain(`returnTo=${encodeURIComponent('/dashboard/settings')}`);
    expect(search).not.toContain('/api/user/self');
  });

  it('顶层旧式路径按未知路由处理', async () => {
    setup('authenticated', '/settings');
    await screen.findByRole('heading', { name: '页面不存在' });
    expect(screen.queryByRole('heading', { name: '个人设置' })).toBeNull();
  });
});
