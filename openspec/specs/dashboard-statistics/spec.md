...
新增 Dashboard 统计接口 SHALL 与现有 `/portal/api/usage/summary`、`/portal/api/usage/timeseries` 并存。本变更 MUST NOT 改变旧接口的路径、参数、响应或前端调用；Dashboard 页面切换到新接口、快捷范围换算、图表展示和下钻行为属于后续 LANG-P2-04。

#### Scenario: P2-03 部署但前端尚未迁移
- **WHEN** 新统计接口已部署而 P2-04 尚未实施
- **THEN** 现有 Dashboard 和旧用量接口继续按原契约工作，新接口可被独立契约测试和调用
